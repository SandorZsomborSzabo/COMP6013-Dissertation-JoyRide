 ...
 *    `GMSNavigationSpeedAlertSeverityUnknown` when the speed limit is unknown or the speed is
 *    invalid. Set to `GMSNavigationSpeedAlertSeverityNotSpeeding` when the driver is not speeding
 *    according to the thresholds configured in `GMSNavigationSpeedAlertOptions.` When the driver
 *    is speeding this will be set to `GMSNavigationSpeedAlertSeverityMinor` or
 *    `GMSNavigationSpeedAlertSeverityMajor` according to the configured thresholds.
 * @param percentageAboveLimit Percentage above the speed limit the driver is currently driving at.
...
/**
 * Called when the current `GMSNavigationNavInfo` is updated. This is called regularly during
 * active navigation.
 *
 * @param navInfo The updated `GMSNavigationNavInfo`.
 */
 ...
 *
 * @param listener An object conforming to the `GMSNavigatorListener` protocol.
 */
 ...
 * continue. Explicitly set `guidanceActive` to NO to stop guidance and remaining time/distance
 * updates.
 *
 ...
 * second.
 *
 * @note This value is ignored if no listeners implement `navigator:didUpdateRemainingTime:`.
 */
 ...
 * to one meter.
 *
 * @note This value is ignored if no listeners implement `navigator:didUpdateRemainingDistance:`.
 */
 ...
 *
 * @return `CLTimeIntervalMax` if no route is available.
 */
 ...
 * device position whilst guidance is active.
 *
 * @return `CLLocationDistanceMax` if no route is available.
 */
 ...
 * device position whilst guidance is active.
 *
 * @return `GMSNavigationDelayCategoryNoData` if no route is available or traffic data is
 * unavailable.
 */
 ...
 * guidance is active.
 *
 * @return `CLTimeIntervalMax` if the provided waypoint is not a destination in the current route.
 */
 ...
 * guidance is active.
 *
 * @return `CLLocationDistanceMax` if the provided waypoint is not a destination in the current
 * route.
 */
 ...
 * `GMSRoadSnappedLocationProvider` and simplified to produce line segments. Note: The system polls
